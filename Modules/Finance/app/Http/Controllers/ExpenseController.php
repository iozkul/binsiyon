...
class ExpenseController extends Controller
{
    ...
    {
        $this->authorize('create expenses'); // veya özel bir 'cancel expenses' yetkisi

        if ($expense->status === 'cancelled') {
            return back()->with('error', 'Bu gider zaten iptal edilmiş.');
        }

        DB::beginTransaction();
        try {
            // 1. Orijinal gideri 'iptal edildi' olarak işaretle
            $expense->status = 'cancelled';
            $expense->save();

            // 2. Ters kayıt (storno) oluştur
            $reversalExpense = $expense->replicate(); // Orijinali kopyala
            $reversalExpense->amount = -$expense->amount; // Tutarı negatife çevir
            $reversalExpense->description = '[İPTAL] ' . $expense->description;
            $reversalExpense->status = 'active'; // Ters kayıt aktiftir
            $reversalExpense->reversal_of_expense_id = $expense->id; // Hangi kaydı iptal ettiğini belirt
            $reversalExpense->save();

            // Burada ilgili muhasebe/kasa/banka işlemleri de tersine çevrilmelidir.
            // Örneğin bir Transaction modeli varsa, ters transaction da oluşturulmalı.

            DB::commit();

            return back()->with('success', 'Gider kaydı başarıyla iptal edildi ve ters kayıt oluşturuldu.');

        } catch (\Exception $e) {
            DB::rollBack();
            return back()->with('error', 'İptal işlemi sırasında bir hata oluştu: ' . $e->getMessage());
        }
    }
}
